...
/*
void benchmark_timer1(void *i, void **o)
{
  nk_fiber_set_vc(vc);
  int a = 0;
  uint64_t start = 0;
  ...
  while (a < 20) {
    start = rdtsc();
    nk_simple_timing_loop(10);
    //nk_fiber_yield();
    end = rdtsc(); 

    nk_vc_printf("Iteration %d ; Time = %d\n", a, (end - start));
    a++;
  }
  nk_vc_printf("Benchmark Timer1 is finished\n");
}
*/


#define SMALL 2000
#define LOOPSMALL 100
...
void benchmark_timer1(void *i, void **o)
{
  ...
  int a = 0;
  uint64_t start = 0;
  uint64_t times[SMALL]; // no malloc?
  
  while (a < SMALL) {
    nk_simple_timing_loop(LOOPSMALL);
    times[a] = rdtsc();
    //nk_fiber_yield();
    a++;
  }
  ...
  int j;
  for (j = 1; j < SMALL ; j++)
    nk_vc_printf("Delta (%d itr. to %d itr.) : %d\n", j - 1, j, (times[j] - times[j - 1]));

  nk_vc_printf("Benchmark Timer1 is finished\n");
}

void benchmark_timer2(void *i, void **o)
{
  nk_fiber_set_vc(vc);
  ...
  nk_vc_printf("Benchmark Timer2 is finished\n");
}


/******************* Test Wrappers *******************/

int test_fiber_benchmark1(){
  nk_fiber_t *first;
  nk_fiber_t *second;
  vc = get_cur_thread()->vc;
  
  nk_fiber_start(benchmark_timer1, 0, 0, 0, 1, &first);
  nk_fiber_start(benchmark_timer2, 0, 0, 0, 1, &second);
